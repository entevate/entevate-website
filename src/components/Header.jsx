...
// Hardcoded fallback used when navData prop is missing (e.g. Sanity
// unavailable at build time or component consumed by a page that didn't
// pass it). Mirrors the state before we wired the navigation singleton.
  ...
  primaryLinks: [
    {
      ...
      hasDropdown: true,
      dropdownItems: [
        { _key: 'd1', label: 'Experiential Branding & Marketing', href: '/experiential' },
        { _key: 'd2', label: 'Operational Intelligence', href: '/operational-intelligence' },
        { _key: 'd3', label: 'Innovation | Ventures | Ecosystem', href: '/innovation' },
      ],
    },
    { _key: 'nav-about', label: 'About Us', href: '/about', external: false, hasDropdown: false },
    { _key: 'nav-work', label: 'Our Work', href: '/our-work', external: false, hasDropdown: false },
    {
      _key: 'nav-momentify',
      label: 'Momentify ↗',
...
const Header = ({ navData }) => {
  ...
  const linkTargetProps = (external) =>
    ...
    <>
      ...
      <div className="mob-drawer" id="mobDrawer" role="dialog" aria-label="Navigation menu" aria-hidden="true">
        <div className="mob-head">
          <a href="/" aria-label="ENTEVATE Home">{logoSvg}</a>
        ...
        <div className="mob-footer">
          <a href={nav.ctaHref || '/contact'} className="mob-cta-btn">
            {nav.ctaLabel || 'Get In Touch'}
          </a>
        </div>
      </div>
      ...
      {/* Desktop Navigation */}
      <nav>
        <a href="/" className="nav-logo">{desktopLogoSvg}</a>

        <ul className="nav-center">
          {nav.primaryLinks?.map((link) =>
            link.hasDropdown ? (
              <li key={link._key} className="has-drop">
        ...
        <div className="nav-right">
          <a href={nav.ctaHref || '/contact'} className="btn-outline">
            {nav.ctaLabel || 'Get In Touch'}
          </a>
